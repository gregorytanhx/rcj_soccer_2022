...
void striker() {
    ...
    if (usingDribbler) {
        angleCorrect();
    } else if (goalCorrect && camera.oppGoalVisible && ballData.visible && (camera.oppGoalAngle < 90 || camera.oppGoalAngle > 270) && millis() - lastOutTime > 1000) {
        aimGoal();
    } else {
        angleCorrect();
    }
...
void goalie() {
    ...
    if (movingSideways) {
        ...
        if (!camera.ownGoalVisible) {
            ...
            if (lineData.onLine && ballData.visible && ((!usingOtherBallData && ballData.dist < 60)||(usingOtherBallData && ballData.dist < 30))) {
                ...
                runningSpeed = min(runningSpeed, 80);
