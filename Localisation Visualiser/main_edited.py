...
class Attacker(Bot):
    ...
    def update_move(self, ball, opp):   
        ...
        if self.ball_dist < 100:
            ...
            self.move_angle = self.ball_deg_angle

        if self.has_ball:
           self.move_angle = self.angle
        #print(ball.x, ball.y)
        print(f"goal angle: {round(self.angle)}, ball angle: {round(self.ball_deg_angle)} move angle: {round(self.move_angle)}, ball distance: {self.ball_dist}, has ball: {self.has_ball}")
        self.move(self.move_angle, self.speed) 
