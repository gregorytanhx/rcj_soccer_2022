...
class Bot(object):
    arrow_img = pygame.image.load("assets/red-up arrow.png")
...
class Attacker(Bot):
    ...
    def update_move(self, ball, opp):   
        ...
        if self.has_ball:
            ...
            ball.hit = False
        print(f"goal angle: {round(self.angle)}, absolute ball angle: {round(self.abs_ball_angle)}, relative ball angle: {round(self.rel_ball_angle)},  move angle: {round(move_angle)}")
