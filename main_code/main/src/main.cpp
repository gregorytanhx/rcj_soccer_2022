...
void trackBall() {
    ...
    if (!camera.ballVisible) {
        ...
        camera.ballAngle = diff.getAngle();
        camera.ballDist = diff.getDistance();
    } 
    float ballOffset;
    if (camera.ballAngle < 180){
        // ball in front
        ballOffset = fmin(camera.ballAngle * 0.95, 90);
    }   else {
        ballOffset = max((360 - camera.ballAngle) * 0.95, -90);
    } 
    float factor = 1 - camera.ballDist / MAX_BALL_DIST;
    float ballMult = fmin(1, BALLMULT_A * exp(factor * BALL_MULT_B));

    robotAngle = camera.ballAngle + ballMult * ballOffset;
...
void getBallPosition() {
    // get absolute position of ball on the field 
    if (camera.ballVisible) {
        Point tmp = Point(camera.ballAngle, camera.ballDist);
