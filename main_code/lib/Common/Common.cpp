...
double rad2deg(double angle) { return angle * 180 / pi; }
...
bool angleIsInside(double angleBoundCounterClockwise,
                   double angleBoundClockwise, double angleCheck) {
...
// normalise to 0 to 1
double norm(float val, int max, int min) { return (val - min) / (max - min); }
