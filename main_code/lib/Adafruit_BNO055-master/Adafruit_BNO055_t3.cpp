/***************************************************************************
 ...
 Updated 2016-03-15: i2c_bus type defined to set choice of Wire or Wire2
...
{
    _iBus = iBus; // ADDED RB 0 for Wire() and 1 for Wire2();
    _sensorID = sensorID;
    _aAddress = aAddress;
...
#if ARDUINO >= 100
        Wire2.write((uint8_t)reg);   /// TO BE MODIFIED? NO
        Wire2.write((uint8_t)value); /// TO BE MODIFIED? NO
#else
...
#if ARDUINO >= 100
        value = Wire2.read(); /// TO BE MODIFIED? NO
#else
...
#endif
        Wire2.endTransmission();
        Wire2.requestFrom(_aAddress, (byte)len); /// TO BE MODIFIED? NO

        for (uint8_t i = 0; i < len; i++)
        {
#if ARDUINO >= 100
            buffer[i] = Wire2.read(); /// TO BE MODIFIED? NO
#else
            buffer[i] = Wire2.readByte(); //// MODIFIE RB receive -> readByte
#endif
        }
